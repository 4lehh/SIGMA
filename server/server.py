    ...
    def init(self): 
        ...
        while self.__state:
            try:
                ...
                # -------------- RATE ADAPTATIVO DE SENSORES -------------- 
                current_rate = float(data_decode["rate"])
                sensor_VPD = float(data_decode["VPD"])
                sensor_id = data_decode["identificador"]

                # si no hay VPD previo, se deja el actual como último VPD
                last_VPD = self.__last_VPDs_per_sensor.get(sensor_id, sensor_VPD) 
                ...
                # --------------- BLOQUE DE ACTUADOR --------------------
                # VPD (Vapor pressure deficit). 0 => germinacion, 1 => vegetativo, 2 => floracion
            ...
            if (current_time - self.last_batch_time) >= self.batch_interval or len(self.__data_buffer) >= 50:
                ...
                # Se reinicia el reloj sin importar si se envió o no, preparándolo para los próximos 5 seg.
